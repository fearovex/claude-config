...
- `/memory-manage`: reads the project from scratch and generates `ai-context/stack.md`, `architecture.md`, `conventions.md`, `known-issues.md`, `changelog-ai.md` with real content
...
| `/memory-manage` generates placeholder stack.md (no package.json) | Edit `ai-context/stack.md` manually with the real stack |
...
3. `/memory-manage` — if `ai-context/` is empty or absent
...
- `/memory-manage`: generates or enriches the `ai-context/` files based on real project code
...
**Command sequence**:
1. `/memory-manage` — regenerates all ai-context/ files from real project state
2. `/project-audit` — verify D2 score improved
3. `/project-fix` — address any remaining issues

**Expected outcome per command**:
- `/memory-manage`: replaces stub files with real content; existing files with substantial content are not overwritten
...
| Failure | Recovery |
|---------|----------|
| `/memory-manage` leaves files as stubs because no source code found | The project may have an unusual structure — edit `ai-context/` files manually using the real code as reference |
| D2 still fails after memory-manage | Check each file manually: does it have > 30 lines? Does it have real content (not just headings)? |
...
| D9 shows `Stack relevance check skipped` | No `ai-context/stack.md` or `package.json` found — run `/memory-manage` first |
...
**Command sequence**:
- For any change: `/sdd-explore <change-name>` → `/sdd-propose <change-name>` → proceed with spec + design + tasks + apply
- After apply: `/sdd-apply <change-name>`

**Expected outcome per command**:
- `/sdd-explore <change>`: investigates the codebase area, documents findings in `exploration.md`
- `/sdd-propose <change>`: creates a proposal document with problem, solution, and success criteria
